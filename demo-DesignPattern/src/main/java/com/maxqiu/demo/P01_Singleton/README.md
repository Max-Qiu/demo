...
这种方式基于`classloder`机制避免了多线程的同步问题，不过`instance`在类装载时就实例化，在单例模式中大多数都是调用`getInstance`方法，但是导致类装载的原因有很多种，因此不能确定有其他的方式（或者其他的静态方法）导致类装载，这时候初始化`instanc`就没有达到懒加载的效果
...
- 优点：解决了线程安全问题
- 缺点：方法同步后效率太低。每个线程在想获得类的实例时候，执行` getInstance()`方法都要进行同步。而这个方法只需要执行一次实例化代码，后面的直接`return`就行了。
- 结论：**不要使用！**
